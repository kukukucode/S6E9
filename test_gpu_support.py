...
def test_gpu_setup_builds_only_missing_cuda_support(monkeypatch):
    ...
    g.ensure_lightgbm_backend('/tmp/prototype.py')
...
def test_gpu_setup_does_not_rebuild_or_fall_back_for_unrelated_failure(monkeypatch, failure):
    monkeypatch.setattr(g.platform, 'system', lambda: 'Linux')
    monkeypatch.setattr(g.shutil, 'which', lambda name, **kwargs: '/usr/bin/' + name)
    calls = []
    def run(command, **kwargs):
...
def test_gpu_disabled_reports_accelerator_before_any_install(monkeypatch):
    monkeypatch.setattr(g.platform, 'system', lambda: 'Linux')
...
def test_working_cuda_build_is_not_reinstalled(monkeypatch):
    monkeypatch.setattr(g.platform, 'system', lambda: 'Linux')
    monkeypatch.setattr(g.shutil, 'which', lambda name, **kwargs: '/usr/bin/' + name)
    calls = []
    def run(command, **kwargs):
